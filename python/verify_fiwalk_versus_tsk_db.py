...
def proc_dfxml(fi):
    ...
    #Why this is complicated: Fiwalk assigns the id (<id>) by incrementing a local variable, and TSK assigns the id ("obj_id") with sqlite3's row counter.  Hence it s impossible to join using only the id variable.
    #   fiwalk_tsk.cpp: file_info("id",next_id++);
    ...
    if not None in [fiwalk_id, fiwalk_name]:
        #Populate fiwalk_id_to_tsk_obj_id
        tsk_obj_id = None
